...
//MANUEL
/**
 * List of possible baryon masses.
 */
enum baryon_masses {
  hydrogen,    /**< _m_H_: scattering off of hydrogen */
  electron,    /**< _m_e_: scattering off of electrons */
  proton       /**< _m_p_: scattering off of protons */
};

enum reio_integrators {
  no_int,         /**< no integrator */
  generic_int,    /**< generic integrator */
  stiff_int,      /**< stiff integrator */
};
//MANUEL
...
//MANUEL
  double mB;            // mass of the baryon, in kg.

  double mx;		        // mass of the DM, in MeV.
  double sigma_0;	      // velocity-independent DM-baryon interaction cross section, in m^2. See 1311.2937 and 1802.06788.
  double pow_n;		      // power of the velocity-dependent DM-baryon interaction cross section. See 1311.2937 and 1802.06788.
  double c_n;		        // n-dependent coefficient for the cross section (c_0 = 1). See 1311.2937 and 1802.06788.
  double sigmav_fo;	    // DM annihilation cross section (in m^3/s) that produces the thermal relic through freeze-out, <sigma v>.
  //double Vrms2;       // the RMS of the DM bulk velocity. CHECK: un-comment for writing it in the "thermodynamics" module.
  ...
  double pref_cx2;// prefactor in front of extra term in cx2, could be 0 or 1; default: 1

  short compute_dm_thermo;// flag for when we want to compute DM thermodynamics

  short is_Tb_blind;// flag for when Tb is blind to Tx
  short early_blind_Tb;// whether during the dmdecoupling routine Tb is evolved like Tg, independently of Tx

  short realistic_Tx;// whether we consider realistic Tx and take into account z_nr

  short ignore_He;// flag that asks whether we ignore the helium nuclei when computing the mass fraction for the baryons that the chiDM scatters off
  short beta_Tg;// flag that asks whether the photoionization coefficient beta is evaluated with the photon temperature, as done in the original RECFAST, instead of with the baryon temperature, as done in the CLASS version of RECFAST.

  enum baryon_masses baryon_mass;

  enum reio_integrators reio_integrator;
//MANUEL
...
//MANUEL
  int index_th_Tx;            // DM temperature \f$ T_x \f$
  int index_th_cx2;           // squared DM sound speed \f$ c_x^2 \f$
  int index_th_Rx;	          // DM-on-baryon momentum exchange rate (units 1/Mpc)
  int index_th_Rxprime;	      // DM-on-baryon heat exchange rate (units 1/Mpc)
  int index_th_dRx;           // derivative wrt conformal time of the scattering rate: d Rx/ d tau
//MANUEL
...
//MANUEL
  double z_theq_ratio;                //the redshift at which there is chiDM-b thermal equilibrium (with Rx_H_ratio)
  double appx_z_thdec;          //(an estimate of) the redshift of thermal decoupling
  double z_thdec;               //the redshift of thermal decoupling
  double z_tcdec;               //the redshift of kinetic decoupling
  double z_nr;                  //redshift at which chiDM (IDM-b) becomes non-relativistic
  double z_fo;                  // redshift of freeze-out
//MANUEL
...
//MANUEL
/**
 * Temporary structure where all the DM decoupling history is defined and stored.
 *
 ...
 */
 ...
 struct dmdecoupling {
   ...
   int index_dm_z;              // redshift \f$ z \f$
   int index_dm_xe;             // ionization fraction \f$ x_e \f$
   int index_dm_dkappadtau;     // baryon temperature \f$ T_b \f$
   int index_dm_Tb;             // baryon equation of state parameter \f$ w_b \f$
   int index_dm_wb;             // squared baryon adiabatic sound speed \f$ c_b^2 \f$
   int index_dm_cb2;            // Thomson scattering rate \f$ d \kappa / d \tau \f$ (units 1/Mpc)
   int index_dm_Tx;             // DM temperature \f$ T_x \f$
   int index_dm_cx2;            // squared DM sound speed \f$ c_x^2 \f$
   int index_dm_Rx;             // DM-on-baryon momentum exchange rate (units 1/Mpc)
   int index_dm_Rxprime;        // DM-on-baryon heat exchange rate (units 1/Mpc)
   int index_dm_dRx;            // derivative wrt conformal time of Rx: d Rx / d tau

   int dm_size;

   int rt_size;                 /**< number of lines (redshift steps) in the table */
   double * dmdecoupling_table; /**< table dmdecupling_table[index_z*pdmde->dm_size+index_dm] with all other quantities (array of size pdmde->rt_size*pdmde->dm_size) */

   short is_Tb_blind;
   short early_blind_Tb;

   double Tnow;   /**< defined as in RECFAST */
   double Nnow;   /**< defined as in RECFAST */
   double dmH0;   /**< defined as in RECFAST */
   double fHe;    /**< defined as in RECFAST */
   double Yp;     /**< defined as in RECFAST */

   double eta_bg; /*baryon-to-photon ratio*/
   ...
   int index_dmde_when_reco_start;    // entry in dmdecoupling table when the recombination is going to start.
...
//MANUEL
  int index_re_Tx;         // DM temperature \f$ T_x \f$
  int index_re_cx2;        // squared DM sound speed \f$ c_x^2 \f$
  int index_re_Rx;	       // DM-on-baryon momentum exchange rate (units 1/Mpc)
  int index_re_Rxprime;	   // DM-on-baryon heat exchange rate (units 1/Mpc)
  int index_re_dRx;        // derivative wrt conformal time of Rx: d Rx / d tau
//MANUEL
...
//MANUEL
  short is_it_saha;// flag that asks whether the Saha approximation is being used
  short beta_Tg;// flag that asks whether the photoionization coefficient beta is evaluated with the photon temperature, as done in the original RECFAST, instead of with the baryon temperature, as done in the CLASS version of RECFAST.
//MANUEL
...
//MANUEL
/**
 * Structure containing the indices and the values of the thermodynamics variables which are integrated over time (as well as their time-derivatives).
 */
...
//MANUEL
  struct thermo_vector * pthv;/**< pointer to vector of integrated perturbations and their time-derivatives */
//MANUEL
};

//MANUEL
struct thermodynamics_parameters_and_workspace_dmdec {

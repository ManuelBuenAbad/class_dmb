...
/* Thomas Tram													*/
/****************************************/
/* Adapted by: Manuel A. Buen-Abad, 2018 */
/****************************************/
/*	This is an variable order, adaptive stepsize ODE evolver for CLASS.
	...
	in "thadjust_stepsize" and is essentially a matrix multiplication. Every times
	there is either a stepsize change, the method order k is changed, or we compute
	a new Jacobian, we must call "thnew_linearisation" to calculate a new LU
	...
	If pth->thermodynamics_verbose > 0, this statistic is printed at the end of
	...
	Then, if jac->use_sparse==_TRUE_, thnumjac will try to construct a sparse matrix from
	the dense matrix. If there are too many nonzero elements in the dense matrix, thnumjac
...
*/
#include "common.h"
#include "thermo_stiff_ndf15.h"
#include "sparse.h"

int thermo_stiff_ndf15(
			...
			double * t_vec,
			int tres,
			int (*output)(double x,double y[],double dy[],void * parameters_and_workspace,
				ErrorMsg error_message),//MANUEL: NOTE: out... is here
	...
	double **dif;
	struct th_jacobian jac;
	struct th_numjac_workspace nj_ws;
	...
	int verbose=0;//MANUEL: ORIGINAL
	// int verbose=1;//MANUEL: CHANGED
  ...
  interpidx=(int*)(tempvec2+neqp);
	...
	class_call(thinitialize_jacobian(&jac,neq,error_message),error_message,error_message);

	/* Initialize workspace for thnumjac: */
	class_call(thinitialize_numjac_workspace(&nj_ws,neq,error_message),error_message,error_message);
	...
	for(ii=0;ii<5;ii++){
		...
		if (verbose > 3)
			printf("invGa[%d] and erconst[%d] are initialized\n",ii,ii);//MANUEL: PRINT:
	}
	...
	for(ii=1;ii<=neq;ii++){
		...
		interpidx[ii]=1;
		if (verbose > 3)
			printf("y[%d]=y_inout[%d]=%lf\n",ii,ii-1,y_inout[ii-1]);//MANUEL: PRINT:
	}
	t0 = x_ini;
	tfinal = x_final;

	if (verbose > 3)
		printf("t0=%f, tfinal=%f, t_vec[0]=%f\n",t0,tfinal,t_vec[0]);//MANUEL: PRINT:

	/* Some CLASS-specific stuff:*/
	next=0;
	while (t_vec[next] < t0) {
		if (verbose > 3)
			printf("(next=%d) and t_vec[next]=%.5e < t0=%.5e\n",next,t_vec[next],t0);//MANUEL: PRINT:
		next++;
	}
  ...
  class_call((*derivs)(t0,y+1,f0+1,parameters_and_workspace_for_derivs,error_message),error_message,error_message);
	if (verbose > 3)
		printf("First call of derivs at t0=%lf\n",t0);//MANUEL: PRINT:
  stepstat[2] +=1;
  if ((tfinal-t0)<0.0){
    tdir = -1;
  }
  ...
  nfenj=0;
  class_call(thnumjac((*derivs),t,y,f0,&jac,&nj_ws,abstol,neq,
  ...
  class_call((*derivs)(t+tdel,y+1,tempvec1+1,parameters_and_workspace_for_derivs,error_message),
	     error_message,error_message);
	if (verbose > 3)
		printf("Second call of derivs at t0=%lf\n",t0);//MANUEL: PRINT:
  stepstat[2] += 1;
  ...
  for(ii=1;ii<=neq;ii++){
    ...
    for(jj=1;jj<=neq;jj++){
      ddfddt[ii]+=(jac.dfdy[ii][jj])*f0[jj];
			if (verbose > 3)
				printf("Have done the jacobian matrix, (ii=%d, jj=%d)\n",ii,jj);//MANUEL: PRINT:
    }
  }
  ...
  h = tdir * absh;
	if (verbose > 3)
		printf("Done calculating initial step, getting ready to do the loop\n");//MANUEL: PRINT:
  /* Done calculating initial step. Get ready to do the loop:*/
  ...
  class_call(thnew_linearisation(&jac,hinvGak,neq,error_message),
	     error_message,error_message);
  stepstat[4] += 1;
  ...
  while (done==_FALSE_){
    ...
    if (((fabs(absh-abshlast)/absh)>1e-6)||(k!=klast)){
      thadjust_stepsize(dif,(absh/abshlast),neq,k);
      hinvGak = h * invGa[k-1];
      nconhk = 0;
      class_call(thnew_linearisation(&jac,hinvGak,neq,error_message),
								 error_message,error_message);
    ...
    for( ; ; ){
      ...
      while(gotynew==_FALSE_){
				/*Compute the constant terms in the equation for ynew. Next FOR lop is just: psi = matmul(dif(:,1:k),(G(1:k) * invGa(k)))*/
				for(ii=1;ii<=neq;ii++){
	  			psi[ii] = 0.0;
	  			for(jj=1;jj<=k;jj++){
				...
				theqvec(pred,ynew,neq);

				/*The difference, difkp1, between pred and the final accepted. ynew is equal to the backward difference of ynew of order k+1. Initialize to zero for the iteration to compute ynew.*/
          ...
          if (verbose > 3)
            printf("Third call of derivs at tnew=%lf\n",tnew);//MANUEL: PRINT:
	  			...
	  			if (jac.use_sparse){
	    			...
	    			theqvec(rhs,del,neq);
	    			thlubksb(jac.LU,neq,jac.luidx,del);
	  			}
				...
				if (tooslow==_TRUE_){
	  			...
	  			if (Jcurrent==_FALSE_){
	    			...
	    			class_test(absh <= hmin, error_message,"Step size too small: step:%g, minimum:%g, in interval: [%g:%g]\n", absh,hmin,t0,tfinal);
	  			}
	  			else{
	    			abshlast = absh;
	  			...
	  			class_call(thnew_linearisation(&jac,hinvGak,neq,error_message),error_message,error_message);
	  			stepstat[4] += 1;
	  			havrate = _FALSE_;
				}
				if (verbose > 3)
					printf("gotynew=%d, done with that loop.\n",gotynew);//MANUEL: PRINT:
      }
      /*end of while loop for getting ynew. difkp1 is now the backward difference of ynew of order k+1. */
      err = 0.0;
      for(jj=1;jj<=neq;jj++){
      ...
      if (err>rtol){
				...
				if (verbose > 1)
					printf("\t!!!!successful step!!!! tnew=%lf, t_vec[next]=%lf\n",tnew,t_vec[next]);//MANUEL: PRINT:
    ...
    while ((next<tres)&&(tdir * (tnew - t_vec[next]) >= 0.0)){
			if (verbose > 3)
				printf("About to call output: t_vec[next]=%lf\n",t_vec[next]);//MANUEL: PRINT:
      /* Do we need to write output? */
      if (tnew==t_vec[next]){
				if (verbose > 3)
					printf("tnew = t_vec[next]=%lf\n",tnew);//MANUEL: PRINT:

				class_call((*output)(t_vec[next],ynew+1,f0+1,parameters_and_workspace_for_derivs,error_message),error_message,error_message);
  			if (verbose > 3)
					printf("First call of output: t_vec[next]=%lf\n",t_vec[next]);//MANUEL: PRINT:
      }
      else {
  			if (verbose > 3)
					printf("tnew=%lf != t_vec[next]=%lf\n",tnew,t_vec[next]);//MANUEL: PRINT:
				/*Interpolate if we have overshot sample values*/
				thinterp_from_dif(t_vec[next],tnew,ynew,h,dif,k,yinterp,ypinterp,yppinterp,interpidx,neq,2);

				if (verbose > 3)
					printf("just interpolated from the dif\n");//MANUEL: PRINT:

				class_call((*output)(t_vec[next],yinterp+1,ypinterp+1,parameters_and_workspace_for_derivs, error_message),error_message,error_message);

  			if (verbose > 3)
					printf("Overshot call of output: t_vec[next]=%lf\n",t_vec[next]);//MANUEL: PRINT:
      }
      next++;
    }
    /** End of output **/
  ...
  thuninitialize_jacobian(&jac);
  thuninitialize_numjac_workspace(&nj_ws);
  return _SUCCESS_;
...
/* "thinterp_from_dif", "theqvec", "thadjust_stepsize", "thcalc_C",           */
/* "thnew_linearisation", "relevant_indices", "thludcmp", "thlubksb".       */
/**********************************************************************/

void theqvec(double *datavec,double *emptyvec, int n){
  int i;
  for(i=1;i<=n;i++){
...
int thcalc_C(struct th_jacobian *jac){
...
int thinterp_from_difold(double tinterp,double tnew,double *ynew,double h,double **dif,int k, double *yinterp,
...
/* Subroutine that interpolates from information stored in dif */
int thinterp_from_dif(double tinterp,
                    double tnew,
                    double *ynew,
                    double h,
                    double **dif,
...
int thadjust_stepsize(double **dif, double abshdivabshlast, int neq,int k){
...
int thnew_linearisation(struct th_jacobian *jac,double hinvGak,int neq,ErrorMsg error_message){
  double luparity, *Ax;
  ...
  if(jac->use_sparse==1){
    ...
    funcreturn = thludcmp(jac->LU,neq,jac->luidx,&luparity,jac->LUw);
    class_test(funcreturn == _FAILURE_,error_message,
	       "Failure in thludcmp. Possibly singular matrix!");
  }
  return _SUCCESS_;
}

/** Helper functions */
int thlubksb(double **a, int n, int *indx, double b[]){
  int i,ii=0,ip,j;
  double sum;
  for (i=1;i<=n;i++) {
    ip=indx[i];
...
int thludcmp(double **a, int n, int *indx, double *d, double *vv){
  int i,imax=0,j,k;
...
int thfzero_Newton(int (*func)(double *x,
                             int x_size,
                             void *param,
                             double *F,
                             ErrorMsg error_message),
  ...
  /** All arrays are indexed as [0, n-1] with the exception of p, indx,
      lu_work and Fjac, since they are passed to thludcmp and thlubksb. */
  ...
  for (k=1;k<=ntrial;k++) {
    ...
    *fevals = *fevals + x_size;
    ...
    thludcmp(Fjac, x_size, indx, &d, lu_work); //Solve linear equations using LU decomposition.
    thlubksb(Fjac, x_size, indx, p);
...
/* "thnumjac", "thinitialize_jacobian", "thuninitialize_jacobian",					*/
/* "thinitialize_numjac_workspace", "thuninitialize_numjac_workspace".		*/
/**********************************************************************/
int thnumjac(
	   int (*derivs)(double x, double * y,double * dy,
			 void * parameters_and_workspace, ErrorMsg error_message),
	   double t, double *y, double *fval,
	   struct th_jacobian *jac, struct th_numjac_workspace *nj_ws,
  ...
  */
  ...
  /*Select an increment del for a difference approximation to
    ...
    gained in previous calls to thnumjac.
  */
  for(j=1;j<=neq;j++){
    if (nj_ws->del[j]==0.0){
      for(;;){
  ...
  /* Adjust fac for next call to thnumjac. */
  for(i=1;i<=neq;i++){
	...
	if ((tmpfac!=fac[j])&&(del2!=0.0)){
	  ...
	  *nfe+=1;
	  ...
	  if(maxval1>=maxval2){
	    ...
	    /* Adjust fac for the next call to thnumjac. */
...
int thinitialize_jacobian(struct th_jacobian *jac, int neq, ErrorMsg error_message){
  int i;
...
int thuninitialize_jacobian(struct th_jacobian *jac){
  free(jac->dfdy[1]);
  free(jac->dfdy);
  free(jac->LU[1]);
  free(jac->LU);
...
int thinitialize_numjac_workspace(struct th_numjac_workspace * nj_ws,int neq, ErrorMsg error_message){
  int i,neqp=neq+1;
...
int thuninitialize_numjac_workspace(struct th_numjac_workspace * nj_ws){
